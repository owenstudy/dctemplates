...
#__02_initODI.format( exp_db = ,date= , project_short = ,date )
              self.__02_initODI = """ 
sqlplus /nolog @01_before_exp.sql>../log/01_before_exp.log
exp {exp_db}/{exp_db}@o46g4 file={exp_db}_{date}.dmp owner={exp_db} log=../log/exp_{exp_db}_{date}.log statistics=none
imp rep_{project_short}/rep_{project_short}@o46g4 file={exp_db}_{date}.dmp log=../log/imp_{exp_db}_{date}.log fromuser={exp_db} touser=rep_{project_short}
...
EXIT;
 """
       ...
       def save_03_after_imp(self):
              ...
              f.write(self.__03_after_imp.format( project_short = self.__project_short, dc_user_base=self.__dc_user_base ,ls_gs=self.__ls_gs ))
              f.close()
